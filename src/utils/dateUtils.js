...
/**
 * Safely parses any date, timestamp, or date string across all browsers (including Safari/iOS Safari).
...
export const parseSafeDate = (val) => {
  ...
  if (typeof val === 'number') {
    if (!isFinite(val)) return new Date();
  ...
  if (typeof val === 'string') {
    ...
    // Normalize "YYYY-MM-DD HH:mm:ss" to "YYYY-MM-DDTHH:mm:ss" for Safari strict ISO parser
...
export const isWithin24Hours = (timestamp) => {
  if (!timestamp) return false;
  const d = parseSafeDate(timestamp).getTime();
...
/**
 * Returns the active user's system/browser timezone string.
 * @returns {string}
 */
