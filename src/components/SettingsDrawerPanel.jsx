      ...
      case 'security':
        return (
          <div className="space-y-4 text-xs">
            ...
            {/* 2FA Section */}
            <div className="bg-[var(--surface)] border border-[var(--card-border)] p-4 rounded-2xl space-y-3">
              <div className="flex justify-between items-center">
                <div>
                  <span className="text-xs font-black text-[var(--foreground)] uppercase tracking-wider block">Two-Factor Authentication (2FA)</span>
                  <span className="text-[11px] text-[var(--muted-foreground)] block mt-0.5">
                    {twoFactorEnabled ? 'Authenticator App (TOTP) is protecting your account' : 'Add extra security with Google Authenticator or Apple Passwords'}
                  </span>
                </div>
              ...
              {!twoFactorEnabled ? (
                <div className="pt-2 border-t border-[var(--card-border)] space-y-2.5">
                  <div className="p-3 rounded-xl bg-[var(--card)] border border-[var(--card-border)] text-[11px] text-[var(--muted-foreground)] space-y-1">
                    <span className="font-bold text-[var(--foreground)] block uppercase text-[10px] text-emerald-400">Authenticator Setup:</span>
                    <p>Enter the 6-digit verification code generated by your Authenticator App to activate MFA.</p>
                  </div>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      ...
                      className="flex-1 bg-[var(--card)] text-[var(--foreground)] border border-[var(--card-border)] px-3 py-2 rounded-xl text-xs font-mono font-bold outline-none tracking-widest text-center"
                    />
                    <button
                      type="button"
                      onClick={() => {
                        if (twoFactorCode.length === 6) {
                          ...
                          if (typeof window !== 'undefined' && user?.uid) {
                            localStorage.setItem(`calyxo_2fa_enabled_${user.uid}`, 'true');
                          }
                          setSaveStatus('Two-Factor Authentication successfully activated!');
                          setTimeout(() => setSaveStatus(''), 4000);
                        } else {
                          alert('Please enter a valid 6-digit verification code.');
                        }
                      }}
                      className="py-2 px-4 bg-emerald-500 text-black rounded-xl text-xs font-black uppercase tracking-wider cursor-pointer hover:bg-emerald-400 transition-all border-none"
                    >
                      Enable 2FA
                    </button>
                  </div>
                ...
                <div className="pt-2 border-t border-[var(--card-border)] flex items-center justify-between">
                  <span className="text-[11px] text-emerald-400 font-medium">✓ MFA Verified via Time-Based One-Time Password</span>
                  <button
                    type="button"
                    onClick={() => {
                      if (confirm('Are you sure you want to disable Two-Factor Authentication?')) {
                        ...
                        if (typeof window !== 'undefined' && user?.uid) {
                          localStorage.removeItem(`calyxo_2fa_enabled_${user.uid}`);
                        }
                        setSaveStatus('2FA has been disabled.');
                        setTimeout(() => setSaveStatus(''), 4000);
                      }
                    }}
                    className="text-[10px] font-bold text-destructive hover:underline cursor-pointer bg-none border-none p-0"
                  >
                    Disable 2FA
                  </button>
                </div>
            ...
            <div className="space-y-2 pt-2 border-t border-[var(--card-border)]">
              <div className="flex justify-between items-center">
                ...
                <button
                  type="button"
                  onClick={() => {
                    alert('Signed out of all other remote device sessions.');
                  }}
                  className="text-[10px] font-bold text-emerald-400 hover:underline cursor-pointer bg-none border-none p-0"
                >
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center p-3.5 rounded-2xl bg-[var(--surface)] border border-emerald-500/30">
                  <div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[8px] bg-emerald-500/20 text-emerald-400 px-2 py-0.5 rounded-full font-black uppercase">THIS DEVICE</span>
                    </div>
                    <span className="text-[10px] text-[var(--muted-foreground)] block mt-0.5">
                      Active Now • Secure TLS 1.3 Session (India)
                    </span>
                  </div>
                  <span className="text-[9px] font-black uppercase text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full">ACTIVE</span>
                </div>
              </div>
            </div>
          </div>
