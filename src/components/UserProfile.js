...
export default function UserProfile({ onNotification }) {
  ...
  const [themeMode, setThemeMode] = useState(() => {
    return useStore.getState().userProfile?.appearance?.themeMode || 'system';
  });
          ...
          >
            ...
            {activePanel === 'account' && (
              <div className="glass p-6 rounded-2xl border border-card-border shadow-md space-y-6">
                ...
                {/* Role Switcher Override for Simulation & Testing */}
                <div className="space-y-3 pt-4 border-t border-card-border">
                  <h4 className="text-xs font-bold text-foreground flex items-center gap-1.5">
                    <Shield className="w-4 h-4 text-acid-green animate-pulse" />
                    Role Management (Testing & Simulation)
                  </h4>
                    ...
                    {['user', 'trainer', 'dietitian', 'admin'].map((r) => {
                      ...
                      return (
                        <button
                          ...
                          type="button"
                          onClick={async () => {
                            const success = await updateUserRole(userId, r);
                            if (success) {
