...
export const toValidUuid = (str) => {
  if (!str || typeof str !== 'string') return '00000000-0000-4000-8000-000000000000';
  ...
  const hex12 = Math.abs(hash * 31).toString(16).padStart(12, '0');
  return `${hex.substring(0, 8)}-4000-8000-0000-${hex12.substring(0, 12)}`;
};
...
export const getFoodLogs = async (userId) => {
  ...
  try {
    ...
    // Normalize timestamps and macro data types
    const remoteLogs = (data || []).map(item => {
      let rawTs = item.timestamp || item.created_at || item.logged_at;
      let tsMs = Date.now();
      if (typeof rawTs === 'number') {
        tsMs = rawTs < 10000000000 ? rawTs * 1000 : rawTs;
      } else if (typeof rawTs === 'string') {
        const clean = rawTs.trim();
        if (/^\d+$/.test(clean)) {
          const num = Number(clean);
          tsMs = num < 10000000000 ? num * 1000 : num;
        } else {
          tsMs = new Date(clean).getTime() || Date.now();
        }
      }
      return {
        ...item,
        name: item.name || item.food_name || "Food Item",
        calories: Number(item.calories) || 0,
        protein: Number(item.protein) || Number(item.protein_g) || 0,
        carbs: Number(item.carbs) || Number(item.carbs_g) || 0,
        fat: Number(item.fat) || Number(item.fat_g) || 0,
        portionWeight: Number(item.portionWeight) || Number(item.serving_size) || 100,
        timestamp: tsMs
      };
    });
...
export const addFoodLog = async (userId, item) => {
  if (!isValidUserId(userId)) {
    console.error("Supabase addFoodLog rejected: invalid user ID provided", userId);
    return item;
  }
  ...
  const logItem = {
    ...item,
    id: item.id && !item.id.includes('food_') ? item.id : generateUuid(),
    userId: validUid,
  ...
  try {
    ...
    const finalItem = { ...logItem, ...data, timestamp: Number(data?.timestamp || logItem.timestamp) };
    setTimeout(() => notifyDataMutation(userId), 150);
    return finalItem;
  } catch (err) {
...
export const deleteFoodLog = async (userId, logId) => {
  ...
  try {
    ...
    if (error) console.warn("Supabase deleteFoodLog delete error, removed locally:", error);
    setTimeout(() => notifyDataMutation(userId), 150);
  } catch (err) {
    console.warn("Supabase deleteFoodLog exception, removed locally:", err);
  }
};
...
export const updateFoodLog = async (userId, logId, updatedItem) => {
  ...
  try {
    ...
    if (error) console.warn("Supabase updateFoodLog update error, updated locally:", error);
    setTimeout(() => notifyDataMutation(userId), 150);
  } catch (err) {
    console.warn("Supabase updateFoodLog exception, updated locally:", err);
  }
};
...
export const getWorkoutLogs = async (userId) => {
  ...
  try {
    ...
    const mergedLogs = mergeLogs(remoteMapped, localLogs);
    const state = getLocalState(userId);
...
export const addWorkoutLog = async (userId, workout) => {
  ...
  const logItem = {
    ...workout,
    id: workout.id && !workout.id.includes('workout_') ? workout.id : generateUuid(),
    userId: validUid,
  ...
  try {
    ...
    try {
      setTimeout(() => {
        try { notifyDataMutation(userId); } catch (e) { console.warn('[dbService] Async sync notification error:', e); }
      }, 150);
    } catch (e) {}
    return finalItem;
  } catch (err) {
...
export const updateWorkoutLog = async (userId, logId, updatedItem) => {
  ...
  try {
    ...
    if (error) console.warn("Supabase updateWorkoutLog update error, updated locally:", error);
    try {
      setTimeout(() => {
        try { notifyDataMutation(userId); } catch (e) { console.warn('[dbService] Async sync notification error:', e); }
      }, 150);
    ...
    console.warn("Supabase updateWorkoutLog exception, updated locally:", err);
  }
};
...
export const deleteWorkoutLog = async (userId, logId) => {
  ...
  try {
    ...
    if (error) console.warn("Supabase deleteWorkoutLog delete error, removed locally:", error);
    try {
      setTimeout(() => {
        try { notifyDataMutation(userId); } catch (e) { console.warn('[dbService] Async sync notification error:', e); }
      }, 150);
    ...
    console.warn("Supabase deleteWorkoutLog exception, removed locally:", err);
  }
};
...
export const getWeightLogs = async (userId) => {
  const localLogs = getLocalState(userId).weightLogs || [];
  if (isMockMode || !userId) {
    return localLogs;
  }
  const validUid = toValidUuid(userId);
  ...
  try {
    ...
    try {
      ...
      const res = await supabase
        .from("user_profiles")
        .select("subscription_plan, email, is_subscribed, subscription_status, photo_url, display_name")
        .eq("id", validUid)
        .maybeSingle();
      upData = res?.data;
...
export const saveUserProfile = async (userId, profile) => {
  ...
  try {
    const extraFields = {
      ...
      photoURL: mergedProfile.photoURL ?? existingBio.photoURL ?? "",
...
export const notifyDataMutation = (userId) => {
  try {
    invalidateUserDataCache(userId);
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('calyxo_data_sync', { detail: { userId } }));
      if ('BroadcastChannel' in window) {
        const syncChannel = new BroadcastChannel('calyxo_data_sync_channel');
        syncChannel.postMessage({ userId, timestamp: Date.now() });
        syncChannel.close();
      }
    }
  } catch (e) {
    console.warn('[dbService] notifyDataMutation warning:', e);
  }
};
...
export const subscribeToUserDataChanges = (userId, onDataChange) => {
  ...
  const channel = supabase
    .channel(`realtime_user_sync_${validUid}`)
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'food_logs', filter: `userId=eq.${validUid}` },
      () => {
        invalidateUserDataCache(userId);
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'workout_logs', filter: `userId=eq.${validUid}` },
      () => {
        invalidateUserDataCache(userId);
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'weight_logs', filter: `userId=eq.${validUid}` },
      () => {
        invalidateUserDataCache(userId);
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'users_metrics', filter: `userId=eq.${validUid}` },
      () => {
        invalidateUserDataCache(userId);
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'user_profiles', filter: `id=eq.${validUid}` },
      () => {
        invalidateUserDataCache(userId);
    ...
    .on(
      'postgres_changes',
      { event: '*', schema: 'public', table: 'subscriptions', filter: `user_id=eq.${validUid}` },
      () => {
        invalidateUserDataCache(userId);
...
export const updateUserAuthProfile = async (displayName, photoURL) => {
  ...
  try {
    const safePhoto = (photoURL && photoURL.length > 2000) ? photoURL.substring(0, 100) + '...' : photoURL;
    await supabase.auth.updateUser({ data: { displayName, photoURL: safePhoto, avatar_url: safePhoto } });
  } catch (e) {
    console.warn('[dbService] Supabase Auth metadata update warning (non-fatal):', e);
  }
};
