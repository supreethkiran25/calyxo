...
import { addFoodLog, getFoodLogs, deleteFoodLog } from '../dbService';

// Fallback Local Foods Database (Instant suggestions)
...
// Initial Weekly Indian Planner templates
const INITIAL_WEEKLY_PLANNER = [
  ...
  },
  {
    ...
    diet: [
      ...
      { category: "Dinner", name: "Grilled Chicken & Veggies", desc: "Grill breast served with stir-fry broccoli and peppers.", calories: 460, protein: 42, carbs: 18, fat: 14 }
    ]
  },
  {
    dayName: "Saturday",
...
function FoodTracker({ userId, userBmi, userGoal, foodLogs, setFoodLogs, onNotification }) {
  // Search state
  ...
  const handleSearch = async (val) => {
    ...
    let apiResults = [];
    try {
      const response = await fetch(`https://world.openfoodfacts.org/cgi/search.pl?search_terms=${encodeURIComponent(val)}&json=true&page_size=8`);
      if (response.ok) {
      ...
      console.warn("Open Food Facts API offline, suggestions loaded from local fallback", err);
    }
  ...
  const logFoodItem = async () => {
    ...
    onNotification(`Logged: ${logItem.name} (+${logItem.calories} kcal)`);
  };

  const handleCustomFoodSubmit = async (e) => {
    e.preventDefault();
    const calories = Math.round(parseFloat(cfCal));
    const protein = parseFloat(parseFloat(cfProt).toFixed(1));
    const carbs = parseFloat(parseFloat(cfCarb).toFixed(1));
    const fat = parseFloat(parseFloat(cfFat).toFixed(1));
    ...
    onNotification(`Logged Custom Food: ${cfName}`);
  };
  ...
  const handleDeleteMeal = async (id) => {
    if (window.confirm("Remove log timeline entry?")) {
      ...
      onNotification("Timeline record deleted.");
    }
  };

  // Log recommended meal directly from weekly planner
  const handleLogSuggestedMeal = async (meal) => {
    ...
    const saved = await addFoodLog(userId, logItem);
    setFoodLogs([saved, ...foodLogs]);
    onNotification(`Logged suggestion: ${meal.name}`);
  };
  ...
  const handleSaveMealEdit = (idx) => {
    ...
    onNotification("Suggested planner item updated.");
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <section className="glass rounded-2xl p-6 relative">
        <h2 className="text-lg font-bold text-white mb-2">Food Compatibility Hub</h2>
        <p className="text-gray-400 text-xs mb-4">Search or create foods to analyze biometric suitability scores</p>
        ...
        <div ref={dropdownRef} className="relative">
          <input 
            ...
            placeholder="Type oats, chicken breast, paneer..."
            className="w-full bg-white/5 border border-white/10 focus:border-neon-green focus:bg-white/10 rounded-full px-5 py-3 text-sm text-white focus:outline-none"
          />

          {showDropdown && searchResults.length > 0 && (
            <div className="absolute top-[calc(100%+8px)] left-0 w-full glass rounded-xl border border-white/10 z-30 max-h-60 overflow-y-auto shadow-2xl">
              {searchResults.map((item, idx) => (
                <div 
                  key={idx}
                  onClick={() => selectFood(item)}
                  className="px-5 py-3.5 border-b border-white/5 last:border-b-0 flex justify-between items-center cursor-pointer hover:bg-neon-green hover:text-black transition-colors"
                >
                  <span className="text-xs font-semibold">{item.name}</span>
                  <span className="text-[10px] opacity-75">{item.calories} kcal / 100g</span>
                </div>
              ))}
            </div>
          )}
        ...
        <div className="flex justify-end mt-3">
          <button 
            ...
            className="text-xs text-neon-green hover:text-white cursor-pointer font-bold uppercase tracking-wider"
          >
      ...
      {/* 2. Custom Food Creator Panel */}
      {showCustomFood && (
        <section className="glass border-neon-green/30 border rounded-2xl p-6">
          <h3 className="text-md font-bold text-neon-green mb-4 font-code">&lt;Create Custom Food Item /&gt;</h3>
          <form onSubmit={handleCustomFoodSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col space-y-1">
                <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Food Name</label>
                <input 
                  type="text" 
                  value={cfName}
              ...
              <div className="flex flex-col space-y-1">
                <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Calories (kcal/100g)</label>
                <input 
                  type="number" 
                  value={cfCal}
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="flex flex-col space-y-1">
                ...
                <input 
                  ...
                  className="bg-white/5 border border-white/10 rounded-xl px-2 py-2 text-xs text-white focus:outline-none focus:border-neon-green"
                  required
                />
              </div>
              <div className="flex flex-col space-y-1">
              ...
              <div className="flex flex-col space-y-1">
                <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Fats (g)</label>
                <input 
                  type="number" 
                  step="0.1"
                  value={cfFat}
            ...
            <div className="grid grid-cols-3 gap-2 border-t border-white/5 pt-3">
              <div className="flex flex-col space-y-1">
                <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Fiber (g)</label>
                <input 
                  type="number" 
                  step="0.1"
                  value={cfFiber}
                  onChange={(e) => setCfFiber(e.target.value)}
                  className="bg-white/5 border border-white/10 rounded-xl px-2 py-2 text-xs text-white"
                />
              </div>
              <div className="flex flex-col space-y-1">
                <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Sugar (g)</label>
                <input 
                  type="number" 
                  step="0.1"
                  value={cfSugar}
              ...
              <div className="flex flex-col space-y-1">
                <label className="text-[9px] text-gray-400 font-bold uppercase tracking-wider">Sodium (mg)</label>
                <input 
                  type="number" 
                  value={cfSodium}
            ...
            <div className="flex justify-end gap-3 pt-2">
              <button 
                ...
                className="btn btn-secondary py-2 px-4 rounded-xl text-xs cursor-pointer"
              >
                Cancel
              </button>
              <button 
                type="submit" 
                className="bg-neon-green text-black font-bold text-xs py-2 px-4 rounded-xl hover:shadow-[0_0_12px_rgba(57,255,20,0.5)] cursor-pointer"
              >
                Save & Log Item
              </button>
            </div>
      ...
      {/* 3. Analysis Card */}
      {analysedFood && (
        <section className="glass border-neon-green/30 border rounded-2xl p-6 space-y-5">
          <div className="flex flex-col items-center justify-center p-4 border-b border-white/5">
            <div className="w-24 h-24 rounded-full border-3 border-neon-green flex flex-col items-center justify-center shadow-[0_0_15px_rgba(57,255,20,0.3)]">
              <span className="text-2xl font-black text-white">{compat.score}%</span>
              <span className="text-[8px] text-gray-400 font-bold uppercase tracking-wider">Score</span>
            </div>
            <h3 className="text-md font-bold text-white mt-3 text-center">{analysedFood.name}</h3>
            <span className="text-[9px] font-bold text-neon-green px-2 py-0.5 rounded-full border border-neon-green bg-neon-green/5 mt-2">
              {compat.rating}
            </span>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <h4 className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-2">Macro-Nutrients (per 100g serving)</h4>
              <div className="grid grid-cols-4 gap-2 text-center text-xs">
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-gray-400">Calories</div><div className="font-bold text-white">{analysedFood.calories} kcal</div></div>
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-accent-blue">Protein</div><div className="font-bold text-white">{analysedFood.protein}g</div></div>
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-accent-yellow">Carbs</div><div className="font-bold text-white">{analysedFood.carbs}g</div></div>
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-accent-pink">Fats</div><div className="font-bold text-white">{analysedFood.fat}g</div></div>
              </div>
            </div>

            <div>
              <h4 className="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-2">Micro-Nutrients & Fiber (per 100g serving)</h4>
              <div className="grid grid-cols-3 gap-2 text-center text-xs">
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-neon-green">Dietary Fiber</div><div className="font-bold text-white">{(analysedFood.fiber || 0).toFixed(1)}g</div></div>
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-gray-400">Sugars</div><div className="font-bold text-white">{(analysedFood.sugar || 0).toFixed(1)}g</div></div>
                <div className="bg-white/3 py-2 rounded-lg"><div className="text-[8px] text-gray-400">Sodium</div><div className="font-bold text-white">{Math.round(analysedFood.sodium || 0)}mg</div></div>
              </div>
            </div>

            <div className="space-y-1">
              <h4 className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Calyxo Suitability analysis</h4>
              <p className="text-gray-300 text-xs leading-relaxed">{compat.reason}</p>
            </div>

            <div className="flex gap-3 items-end pt-3">
              <div className="flex flex-col space-y-1 flex-1">
                <label className="text-[10px] text-gray-400 uppercase font-bold tracking-wider">Log Weight (g)</label>
                <input 
                  type="number" 
                  value={portion} 
                  onChange={(e) => setPortion(Number(e.target.value))}
                  className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-neon-green"
                />
              </div>
              <button 
                onClick={logFoodItem}
                className="bg-neon-green text-black font-bold text-xs py-2.5 px-4 rounded-xl cursor-pointer hover:shadow-[0_0_12px_rgba(57,255,20,0.5)] transition-all h-[36px] flex items-center justify-center"
              >
                Log Portion
              </button>
            </div>
      ...
      <section className="glass rounded-2xl p-6">
        <h2 className="text-lg font-bold text-white mb-4">Logged Intake Timeline</h2>
            ...
            foodLogs.map((item, idx) => (
              <div key={idx} className="flex justify-between items-center bg-white/3 border border-white/5 px-4 py-3 rounded-xl">
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-white">{item.name} <span className="text-[9px] text-gray-400 font-medium">({item.portionWeight}g)</span></span>
                  <span className="text-[9px] text-gray-400 mt-0.5">P: {item.protein}g | C: {item.carbs}g | F: {item.fat}g | Fiber: {item.fiber || 0}g</span>
                </div>
                <div className="flex items-center gap-3">
                  <span className="text-xs font-bold text-neon-green">+{item.calories} kcal</span>
                  <button 
      ...
      {/* 5. Weekly Diet Suggestions Tabs */}
      <section className="glass rounded-2xl p-6">
        ...
        <div className="space-y-4">
          <h3 className="text-xs text-gray-400 font-bold uppercase tracking-wider">Weekly Calyxo Recipe Recommendations</h3>
          {weeklyPlanner[activeDay].diet.map((meal, idx) => (
            <div key={idx} className="border-b border-white/5 last:border-b-0 pb-3 last:pb-0">
              {editingMealIndex === idx ? (
                // Edit Interface
                <div className="space-y-3 p-3 bg-white/3 rounded-xl border border-white/10">
                  <input 
                    type="text" 
                  ...
                  <div className="grid grid-cols-4 gap-2 text-center text-[10px]">
                    <div>Calories<input type="number" value={editMealFields.calories} onChange={(e) => setEditMealFields({ ...editMealFields, calories: e.target.value })} className="w-full bg-black/40 text-center rounded py-0.5 mt-1 border border-white/10" /></div>
                    <div>Prot<input type="number" value={editMealFields.protein} onChange={(e) => setEditMealFields({ ...editMealFields, protein: e.target.value })} className="w-full bg-black/40 text-center rounded py-0.5 mt-1 border border-white/10" /></div>
                    <div>Carbs<input type="number" value={editMealFields.carbs} onChange={(e) => setEditMealFields({ ...editMealFields, carbs: e.target.value })} className="w-full bg-black/40 text-center rounded py-0.5 mt-1 border border-white/10" /></div>
                    <div>Fat<input type="number" value={editMealFields.fat} onChange={(e) => setEditMealFields({ ...editMealFields, fat: e.target.value })} className="w-full bg-black/40 text-center rounded py-0.5 mt-1 border border-white/10" /></div>
                  </div>
                  <div className="flex justify-end gap-2 pt-1">
                    <button onClick={() => setEditingMealIndex(null)} className="text-[10px] text-gray-400 py-1 px-3 bg-white/5 rounded-lg">Cancel</button>
                    <button onClick={() => handleSaveMealEdit(idx)} className="text-[10px] text-black bg-neon-green py-1 px-3 rounded-lg font-bold">Save</button>
                  </div>
                </div>
              ) : (
                // Display Interface
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] text-neon-green font-bold uppercase">{meal.category}</span>
                      <h4 
                        onClick={() => handleStartEditMeal(idx, meal)}
                        className="text-xs font-bold text-white border-b border-dashed border-gray-600 cursor-pointer hover:text-neon-green"
                      >
                        {meal.name}
                      </h4>
                    </div>
                    <p className="text-[10.5px] text-gray-400 mt-1 leading-relaxed">{meal.desc}</p>
                  ...
                  <button 
                    ...
                    className="w-7 h-7 rounded-full bg-neon-green/10 border border-neon-green/20 hover:bg-neon-green hover:text-black flex items-center justify-center cursor-pointer transition-colors text-neon-green font-bold text-xs"
                  >
                    +
                  </button>
                </div>
